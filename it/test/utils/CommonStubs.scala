...
trait CommonStubs {
  ...
  def stubAuth(zRef: String = TestConstants.validZReference): Unit = {
    val validEnrolment =
      s"""{
  ...
  def stubReportingWindowOverride(status: ResponseDefinitionBuilder, zRef: String): Unit =
    stubFor(
      put(urlEqualTo(s"/reporting-window-override/$zRef"))
        .willReturn(status)
    )
